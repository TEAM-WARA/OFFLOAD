...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.httpBasic().disable()
                ...
                .antMatchers("/auth/**", "/image/**", "/api-docs/**" , "/swagger-ui/**","/swagger-resources/**", "/v2/api-docs/**", "/store/**").permitAll()
                // 인증이 완료되면 모든 api 접근 허용
                // 판매자만 접근할 수 있는 경로 설정
                //.antMatchers("/store/**").hasRole("SELLER")
                // 판매자와 유저 모두 접근할 수 있는 경로 설정
                .antMatchers("/storage/**").hasAnyRole("SELLER", "USER", "ADMIN")
                        // 개발용 설정
